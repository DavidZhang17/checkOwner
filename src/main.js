...
// 从 params.json 读取查询配置
const paramsFilePath = path.join(__dirname, "data", "params.json")
let querieList = []

try {
	const paramsData = JSON.parse(fs.readFileSync(paramsFilePath, "utf-8"))
	// 将 params.json 格式转换为 querieList 格式
	querieList = paramsData.map((item) => {
		const templateContent = JSON.parse(item.templateContent)
		return {
	...
	console.log(`✓ 成功加载查询配置文件，共 ${querieList.length} 个查询条件`)
} catch (error) {
	console.error(`❌ 无法读取查询配置文件 ${paramsFilePath}:`, error.message)
	console.error("请确保 params.json 文件存在且格式正确")
	process.exit(1)
}
...
;(async () => {
	try {
		console.log(`\n==================== 开始处理 ====================`)
		console.log(`共有 ${querieList.length} 个查询条件需要处理`)
		console.log(`输出目录: ${outputDir}`)
		console.log("================================================\n")

		// 循环处理每个查询条件
		for (let i = 0; i < querieList.length; i++) {
			const queryConfig = querieList[i]
			const { fileName, query } = queryConfig

			console.log(`\n[${i + 1}/${querieList.length}] 开始处理: ${fileName}`)
			console.log(`起始页码: ${query.pageNum}`)
			console.log(`每页大小: ${query.pageSize}`)
			console.log(`数据将写入Excel第 ${(query.pageNum - 1) * query.pageSize + 2} 行开始`)
			if (query.pageNum > 1) {
			...
			console.log(`✓ [${i + 1}/${querieList.length}] 完成: ${fileName}`)

			// 如果不是最后一个，等待一段时间
			if (i < querieList.length - 1) {
				console.log(`\n休息 5 秒后继续下一个查询...\n`)
		...
		console.log(`\n==================== 全部完成 ====================`)
		console.log(`已成功处理 ${querieList.length} 个查询条件`)
...
async function fetchProjectNames(personId) {
	try {
		const response = await request.post("/jian-butler-owner-biz/person/achievement/pageProjectWinningByPerson", {
			personId: personId,
			pageSize: 5,
			pageNum: 1,
		})

		if (response.data && response.data.status && response.data.data && response.data.data.records) {
			// 提取projectName字段，并去重
...
async function fetchPersonnelData(queryCondition, pageNum) {
	try {
		...
		const response = await request.post("/jian-butler-owner-biz/personnel/pagePersonnel", queryToSend)
		...
		if (record.uuid) {
			try {
				...
				if ((i + 1) % 1000 === 0 && i + 1 < records.length) {
					console.log(`  已处理 ${i + 1} 条记录，休息 1 分钟...`)
...
async function fetchAndSaveData(outputPath, queryCondition, fileName) {
	...
	try {
		...
		console.log(`第 ${startPageNum} 页: 获取到 ${firstPage.records.length} 条人员记录`)

		// 为第一页数据获取项目名称
		await enrichRecordsWithProjects(firstPage.records)

		// 生成输出文件路径（保存到日期文件夹）
		const outputPath2 = path.join(outputPath, `${fileName}-(${firstPage.records.length}条).xlsx`)
		...
		console.log(`从第 ${startPageNum} 页继续，之前已完成 ${alreadyProcessed} 条记录`)
		...
		// 计算需要处理的最大页数
		const maxPageToProcess = Math.min(totalPages, Math.ceil(MAX_RECORDS / pageSize))

		console.log(`休息 1 分钟后继续...\n`)
		// await new Promise((resolve) => setTimeout(resolve, 60000));

		// 获取剩余页数据
		for (let pageNum = startPageNum + 1; pageNum <= maxPageToProcess; pageNum++) {
			// 检查是否已达到5000条限制
			...
			console.log(`第 ${pageNum} 页: 获取到 ${pageData.records.length} 条人员记录`)
...
async function convertJsonToExcel(outputPath, queryCondition, fileName) {
	try {
		...
		console.log(`数据将从第 ${(queryCondition.pageNum - 1) * queryCondition.pageSize + 2} 行开始写入`)
